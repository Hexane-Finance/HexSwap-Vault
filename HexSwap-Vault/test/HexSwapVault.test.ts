...
const HexaneToken = artifacts.require("HexaneToken");
const SyrupBar = artifacts.require("SyrupBar");
const MasterChef = artifacts.require("MasterChef");
const HexaneVault = artifacts.require("HexaneVault");
const VaultOwner = artifacts.require("VaultOwner");
const MockERC20 = artifacts.require("MockERC20");

const REWARDS_START_BLOCK = 100;

contract("HexaneVault", ([owner, admin, treasury, user1, user2, user3, harvester]) => {
  let vault, masterchef, Hexane, syrup, rewardsStartBlock;
  ...
  async function getUserInfo(user: any) {
    ...
    return {
      ...
      HexaneAtLastUserAction: userInfo[2],
  ...
  beforeEach(async () => {
    ...
    Hexane = await HexaneToken.new({ from: owner });
    syrup = await SyrupBar.new(Hexane.address, { from: owner });
    masterchef = await MasterChef.new(Hexane.address, syrup.address, owner, ether("1"), rewardsStartBlock, {
      from: owner,
    }); // 1 Hexane per block, starts at +100 block of each test
    vault = await HexaneVault.new(Hexane.address, syrup.address, masterchef.address, admin, treasury, { from: owner });

    await Hexane.mint(user1, ether("100"), { from: owner });
    await Hexane.mint(user2, ether("100"), { from: owner });
    await Hexane.mint(user3, ether("100"), { from: owner });
    await Hexane.approve(vault.address, ether("1000"), { from: user1 });
    await Hexane.approve(vault.address, ether("1000"), { from: user2 });
    await Hexane.approve(vault.address, ether("1000"), { from: user3 });
    await Hexane.transferOwnership(masterchef.address, { from: owner });
    await syrup.transferOwnership(masterchef.address, { from: owner });
  });

  it("Initialize", async () => {
    assert.equal(await Hexane.balanceOf(vault.address), 0);
    assert.equal(await Hexane.balanceOf(vault.address), 0);
    assert.equal(await vault.token(), Hexane.address);
  ...
  it("Should deposit funds, withdraw funds and get shares without rewards", async () => {
    ...
    assert.equal((await Hexane.balanceOf(user1)).toString(), ether("100").toString());
    assert.equal((await Hexane.balanceOf(user2)).toString(), ether("100").toString());
    assert.equal((await Hexane.balanceOf(user3)).toString(), ether("100").toString());

    await vault.deposit(ether("10"), { from: user1 });
    await vault.deposit(ether("20"), { from: user2 });
    await vault.deposit(await Hexane.balanceOf(user3), { from: user3 });

    // Rewards not started yet, so no rewards are earned, so all ratios should be 1:1
    // 1 share should equal to 1 Hexane, pricePerFullShare should equal to 1
    ...
    let user1Hexane = (await getUserInfo(user1)).HexaneAtLastUserAction;
    let user2Hexane = (await getUserInfo(user2)).HexaneAtLastUserAction;
    let user3Hexane = (await getUserInfo(user3)).HexaneAtLastUserAction;
    ...
    assert.equal(user1Hexane.toString(), ether("10").toString());
    assert.equal(user2Hexane.toString(), ether("20").toString());
    assert.equal(user3Hexane.toString(), ether("100").toString());
    ...
    assert.equal((await Hexane.balanceOf(user1)).toString(), ether("90").toString());
    assert.equal((await Hexane.balanceOf(user2)).toString(), ether("80").toString());
    assert.equal((await Hexane.balanceOf(user3)).toString(), ether("0").toString());
    ...
    assert.equal((await Hexane.balanceOf(user1)).toString(), ether("80").toString());
    assert.equal((await Hexane.balanceOf(user2)).toString(), ether("60").toString());
    assert.equal((await Hexane.balanceOf(user3)).toString(), ether("0").toString());
    ...
    assert.equal((await getUserInfo(user1)).HexaneAtLastUserAction, 0);
    assert.equal((await getUserInfo(user2)).HexaneAtLastUserAction, 0);
    assert.equal((await getUserInfo(user3)).HexaneAtLastUserAction, 0);
  ...
  it("Should deposit funds, withdraw funds and get shares with rewards", async () => {
    ...
    // 1 share should be > 1 Hexane, pricePerFullShare should equal be > 1
    let tx = await vault.deposit(ether("10"), { from: user1 }); // Vault receives 0 Hexane pending reward
    user1Shares = (await getUserInfo(user1)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    tx = await vault.deposit(ether("20"), { from: user2 }); // Vault receives 1 Hexane pending reward
    user2Shares = (await getUserInfo(user2)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    tx = await vault.deposit(await Hexane.balanceOf(user3), { from: user3 }); // Deposits 100 + 1 available, Vault receives 0.99999999999 Hexane pending reward
    user3Shares = (await getUserInfo(user3)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    // Repeated deposit
    tx = await vault.deposit(ether("10"), { from: user1 }); // Vault receives 0.999999999966 Hexane pending reward
    user1Shares = (await getUserInfo(user1)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    tx = await vault.deposit(ether("20"), { from: user2 }); // Vault receives 0.999999999981929578 Hexane pending reward
    user2Shares = (await getUserInfo(user2)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    assert.equal(await vault.totalShares(), 0);
    assert.equal((await Hexane.balanceOf(user1)).toString(), ether("100.824657119370218484").toString()); // Expect > 100
    assert.equal((await Hexane.balanceOf(user2)).toString(), ether("101.807758814702448003").toString()); // Expect > user1Shares
    assert.equal((await Hexane.balanceOf(user3)).toString(), ether("105.367584065575769956").toString()); // Expect > user2Shares
  });
  ...
  it("Should withdraw with withdraw fee", async () => {
    ...
    // await vault.setWithdrawFeePeriod(time.duration.hours(9), { from: admin });
    // Withdraw fee: 0.1%
    assert.equal(await Hexane.balanceOf(user1), ether("100").toString());
    assert.equal(await Hexane.balanceOf(treasury), 0);

    // Time travel to start of rewards
    await time.advanceBlockTo(rewardsStartBlock);

    await vault.deposit(ether("10"), { from: user1 }); // lastDepositedAt starts
    assert.equal(await Hexane.balanceOf(user1), ether("90").toString());
    assert.equal((await vault.available()).toString(), ether("0").toString());
    assert.equal((await vault.balanceOf()).toString(), ether("10").toString());
    let balance = await Hexane.balanceOf(user1);
    ...
    assert.equal((await Hexane.balanceOf(user1)).toString(), balance.add(amountForShares).sub(withdrawFee).toString());
    assert.equal((await Hexane.balanceOf(treasury)).toString(), withdrawFee.toString());
    balance = await Hexane.balanceOf(user1);
    ...
    assert.equal((await Hexane.balanceOf(user1)).toString(), balance.add(amountForShares).toString());
    assert.equal((await Hexane.balanceOf(treasury)).toString(), withdrawFee.toString()); // No change
  });
  ...
  it("Should harvest and reinvest funds", async () => {
    ...
    await vault.deposit(ether("10"), { from: user1 }); // Vault receives 0 Hexane pending reward
    await vault.deposit(ether("20"), { from: user2 }); // Vault receives 1 Hexane pending reward

    assert.equal((await vault.available()).toString(), ether("1").toString());
    assert.equal((await vault.balanceOf()).toString(), ether("31").toString());

    let pendingHexane = ether("0.99999999999");
    await vault.harvest({ from: harvester }); // Receives 0.99999999999 pending Hexane reward

    assert.equal((await vault.available()).toString(), 0);
    assert.equal((await vault.balanceOf()).toString(), ether("31").add(pendingHexane).toString());
    const balance = await vault.balanceOf();

    pendingHexane = ether("0.9999999999996875");
    const tx = await vault.harvest({ from: harvester }); // Receives 0.9999999999996875 pending Hexane reward
    ...
    assert.equal((await vault.available()).toString(), 0);
    assert.equal((await vault.balanceOf()).toString(), balance.add(pendingHexane).toString());
  });

  it("Should harvest with performance and call fees", async () => {
    // Performance fee: 2%, Call fee: 0.25%
    assert.equal(await Hexane.balanceOf(treasury), 0);
    assert.equal(await Hexane.balanceOf(harvester), 0);
    ...
    let tx = await vault.harvest({ from: harvester }); // Receives 0.99999999999 pending Hexane reward
    ...
    assert.equal((await Hexane.balanceOf(treasury)).toString(), treasuryFee.toString());
    assert.equal((await Hexane.balanceOf(harvester)).toString(), harvesterFee.toString());
    assert.equal(await vault.available(), 0);
    assert.equal(
      (await vault.balanceOf()).toString(),
      ether("31").add(pending).sub(treasuryFee).sub(harvesterFee).toString()
    );
    let treasuryTotal = await Hexane.balanceOf(treasury);
    let harvesterTotal = await Hexane.balanceOf(harvester);
    ...
    tx = await vault.harvest({ from: harvester }); // Receives 0.99999999999 pending Hexane reward
    ...
    assert.equal((await Hexane.balanceOf(treasury)).toString(), treasuryTotal.add(treasuryFee).toString());
    assert.equal((await Hexane.balanceOf(harvester)).toString(), harvesterTotal.add(harvesterFee).toString());
    assert.equal(await vault.available(), 0);
    assert.equal(
      (await vault.balanceOf()).toString(),
  ...
  it("Should withdraw non-Hexane and non-syrup tokens inCaseTokensGetStuck", async () => {
    await vault.deposit(ether("10"), { from: user1 });

    await expectRevert(vault.inCaseTokensGetStuck(Hexane.address), "admin: wut?");
    await expectRevert(vault.inCaseTokensGetStuck(syrup.address), "admin: wut?");
    await expectRevert(
      vault.inCaseTokensGetStuck(Hexane.address, { from: admin }),
    ...
    // Send non-Hexane/non-syrup tokens to vault (supposedly by mistake)
    const mockHexane = await HexaneToken.new({ from: owner });
    await mockHexane.mint(user1, ether("100"), { from: owner });
    await mockHexane.transfer(vault.address, ether("99"), { from: user1 });
    assert.equal((await mockHexane.balanceOf(vault.address)).toString(), ether("99").toString()); // Vault has 99 mockHexane

    await vault.inCaseTokensGetStuck(mockHexane.address, { from: admin });
    assert.equal((await mockHexane.balanceOf(vault.address)).toString(), ether("0").toString()); // Vault has 0 mockHexane
    assert.equal((await mockHexane.balanceOf(admin)).toString(), ether("99").toString()); // Admin now has 99 mockHexane
  });
  ...
  it("Should disallow deposits and harvest when paused", async () => {
    ...
    await vault.deposit(ether("10"), { from: user1 });
    await vault.deposit(await Hexane.balanceOf(user1), { from: user1 });
    await vault.harvest();
  });
