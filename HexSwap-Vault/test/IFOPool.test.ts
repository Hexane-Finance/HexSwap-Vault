...
const HexaneVault = artifacts.require("IFOPool");
...
contract("IFOPool", ([owner, admin, treasury, user1, user2, user3, harvester]) => {
  ...
  beforeEach(async () => {
    ...
    vault = await HexaneVault.new(Hexane.address, syrup.address, masterchef.address, admin, treasury, 1001, 1020, {
      from: owner,
    });

    await Hexane.mint(user1, ether("1000"), { from: owner });
    await Hexane.mint(user2, ether("1000"), { from: owner });
    await Hexane.mint(user3, ether("1000"), { from: owner });
    await Hexane.approve(vault.address, ether("100000"), { from: user1 });
  ...
  it("Should deposit 10,20,40 Hexanes at block 2 8 14 and final with credit around 34.73 as case 1", async () => {
    const snapshotA = await snapshot();
    try {
      ...
      assert.equal((await Hexane.balanceOf(user1)).toString(), ether("1000").toString());
  ...
  it("Should deposit 200 Hexanes at block 17 and final with credit around 31.57 as case 2", async () => {
    const snapshotA = await snapshot();
    try {
      await vault.setPerformanceFee(200, { from: admin });
      await vault.setCallFee(20, { from: admin });
  ...
  it("Should deposit 50 Hexane at block 2 and final with credit around 47.36 as case 4", async () => {
    const snapshotA = await snapshot();
    try {
      await vault.setPerformanceFee(200, { from: admin });
      await vault.setCallFee(20, { from: admin });
  ...
  it("Should user1 deposit at block 2, 3, 14 user2 deposit at 3 withdraw at 8 and final credit is user1 34.76 user2 26.31", async () => {
    ...
    try {
      ...
      assert.equal((await Hexane.balanceOf(user1)).toString(), ether("1000").toString());
      assert.equal((await Hexane.balanceOf(user2)).toString(), ether("1000").toString());
  ...
  it("Should deposit 10,20,40 Hexanes at block 2 8 14 and final with credit around 34.73 as case 1 with updateEndBlock", async () => {
    const snapshotA = await snapshot();
    try {
      await vault.setPerformanceFee(200, { from: admin });
      await vault.setCallFee(20, { from: admin });
  ...
  it("Should deposit a small num of Hexane and share not equal to 0", async () => {
    const snapshotA = await snapshot();
    try {
      await vault.setPerformanceFee(200, { from: admin });
      await vault.setCallFee(20, { from: admin });
  ...
  it("Should deposit 10,20,40 Hexanes at block 2 8 14 and final with credit around 34.73 as case 1 with multiple updateEndBlock", async () => {
    const snapshotA = await snapshot();
    try {
      await vault.setPerformanceFee(200, { from: admin });
      await vault.setCallFee(20, { from: admin });
      ...
      console.log(
        "Hexane of masterchef : %s ",
        ethers.utils.formatEther((await Hexane.balanceOf(masterchef.address)).toString())
      );
      ...
      console.log("Hexane of user1 : %s ", ethers.utils.formatEther((await Hexane.balanceOf(user1)).toString()));
