...
contract IFOPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 HEXANEAtLastUserAction; // keeps track of HEXANE deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }
    //IFO
    struct UserIFOInfo {
        // ifo valid period is current block between startblock and endblock
        uint256 lastActionBalance; // staked HEXANE numbers (not include compoud HEXANE) at last action
        uint256 lastValidActionBalance; // staked HEXANE numbers in ifo valid period
    ...
    IERC20 public immutable token; // HEXANE token
    IERC20 public immutable receiptToken; // sHEXANE token
    ...
    /**
     * @notice Constructor
     * @param _token: HEXANE token contract
     * @param _receiptToken: sHEXANE token contract
    ...
    /**
     * @notice Deposits funds into the HEXANE Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in HEXANE)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.HEXANEAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     * @param _lastValidActionBalance: staked HEXANE number at last action.
     ...
     * @param _amount:the HEXANE amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as HEXANE Vault withdraw
    ...
    /**
     * @notice Reinvests HEXANE tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the HEXANE Vault
     */
     ...
     * @return Expected reward to collect in HEXANE
     */
    function calculateHarvestHEXANERewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingHEXANE(0, address(this));
     ...
     * @return Returns total pending HEXANE rewards
     */
    function calculateTotalPendingHEXANERewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingHEXANE(0, address(this));
